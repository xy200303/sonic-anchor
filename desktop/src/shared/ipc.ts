...
export interface CommentConfig {
  /** 自建后端公开地址，例如 https://live.example.com */
  backendUrl: string
  /** 桌面端接入密钥，后续可升级为设备登录凭证 */
  desktopKey: string
  /** 多应用场景下指定抖音 AppID，留空使用后端第一个启用应用 */
  appId: string
  /** 开发模式：使用内置评论模拟器（不连抖音） */
  devSimulate: boolean
}
...
export const DEFAULT_CONFIG: AppConfig = {
  ...
  comment: { backendUrl: 'http://localhost:8090', desktopKey: '', appId: '', devSimulate: false },
  live: { activePlanId: '' }
}
