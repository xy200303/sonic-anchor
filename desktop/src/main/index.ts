...
import { loadConfig } from './modules/store/config'
import { commentGateway } from './modules/comment/gateway'
import { updateDouyinLaunchToken } from './modules/comment/douyinClient'
...
app.whenReady().then(() => {
  protocol.handle('media', (request) => {
    ...
    if (url.hostname === 'local') {
      ...
      const allowedPath = loadConfig().bgm.path
      const normalize = (value: string): string => {
        const normalized = resolve(value)
        return process.platform === 'win32' ? normalized.toLowerCase() : normalized
      }
      if (!allowedPath || normalize(requestedPath) !== normalize(allowedPath)) {
        return new Response('Forbidden', { status: 403 })
      }
      return net.fetch(pathToFileURL(requestedPath).toString())
    }

    const requestedName = pathname.replace(/^\//, '')
    if (!requestedName || requestedName !== basename(requestedName)) {
      return new Response('Forbidden', { status: 403 })
    }
    return net.fetch(pathToFileURL(join(audioCacheDir(), requestedName)).toString())
  })
})
...
if (!gotLock) {
  ...
  app.on('second-instance', (_event, commandLine) => {
    const wasCommentGatewayRunning = commentGateway.status().running
    if (updateDouyinLaunchToken(commandLine) && wasCommentGatewayRunning) {
      commentGateway.stop()
      void commentGateway.start()
    }
