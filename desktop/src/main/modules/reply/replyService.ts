...
import { getComment, insertReplyLog, listPlans, searchKnowledge } from '../store/db'
...
class ReplyService {
  ...
  handleComment(comment: CommentEvent): void {
    if (!this.autoReply || comment.kind !== 'comment') return
  ...
  private async produce(comment: CommentEvent, intent: IntentType): Promise<void> {
    ...
    if (cached && Date.now() - cached.at < QUESTION_CACHE_MS) {
      const item = this.makeItem(comment, cached.text, intent, true)
      item.status = 'synthesizing'
      this.push(item)
      void this.prepareAudio(item, cached.text)
      return
    }
  ...
  private async generateText(comment: CommentEvent, intent: IntentType): Promise<string> {
    const activePlanId = loadConfig().live.activePlanId
    const activeProductId = listPlans().find((plan) => plan.id === activePlanId)?.productId ?? undefined
    const kb = searchKnowledge(comment.content, 3, activeProductId)
    const kbText = kb
...
3. 不承诺知识库之外的参数与售后政策。`
  ...
  private async prepareAudio(item: ReplyItem, text: string): Promise<void> {
    try {
      if (this.aiMuted) {
        item.speak = false
      } else {
    ...
    const comment =
      ...
      (commentId === 'manual'
        ? {
            ...
            sessionId: streamService.getSessionId(),
            author: '主播插话',
            authorId: 'manual',
            content: text,
