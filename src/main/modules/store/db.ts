...
/** 轻量知识库检索：按关键词在卖点/FAQ 里匹配（FTS 前置方案，词条短命中率高） */
export function searchKnowledge(query: string, limit = 3): Product[] {
  const kw = query.replace(/[%_]/g, '').slice(0, 20)
  if (!kw) return []
  return run(
    (d) =>
      d
        .prepare(
          `SELECT * FROM products
           WHERE name LIKE @like OR selling_points LIKE @like OR faq LIKE @like
